...
class AirCargoProblem(Problem):
    ...
    def get_actions(self):
            ...
            for c in self.cargos: 
                for p in self.planes: 
                    for a in self.airports: 
                        action_expr = expr("Load({}, {}, {})".format(c, p, a))
                        precond_pos = [expr("At({}, {})".format(p, a)), 
                                       expr("At({}, {})".format(c, a))]
                        precond_neg = []
                        effect_add = [expr("In({}, {})".format(c, p))]
                        effect_rem = [expr("At({}, {})".format(c, a))]
                        one_load = Action(action_expr, [precond_pos, precond_neg], [effect_add, effect_rem])
        ...
        def unload_actions():
            ...
            for c in self.cargos: 
                for p in self.planes: 
                    for a in self.airports: 
                        action_expr = expr("Unload({}, {}, {})".format(c, p, a))
                        precond_pos = [expr("At({}, {})".format(p, a)), 
                                       expr("In({}, {})".format(c, p))]
                        precond_neg = []
    ...
    def actions(self, state: str) -> list:
        ...
        possible_actions = []
        kb = PropKB()
        # Only positive sentences are needed, because state descriptions are very exhaustive
        # Any literal not in positive list must be in negative list 
        kb.tell(decode_state(state, self.state_map).pos_sentence())
        for each_action in self.actions_list: 
            is_possible = True
            for each_cond in each_action.precond_pos: 
                if each_cond not in kb.clauses: 
                    is_possible = False
                    break
            if not is_possible: continue
            for each_cond in each_action.precond_neg: 
                if each_cond in kb.clauses: 
                    is_possible = False
                    break
            if is_possible: possible_actions.append(each_action)
    ...
    def h_ignore_preconditions(self, node: Node):
        ...
        # One action can achieve at maximum one goal in current problem setting
        # This function is thus simplied to very similar as goal_test()
        for clause in self.goal:
            if clause not in kb.clauses: count += 1        
        return count
...
def air_cargo_p3() -> AirCargoProblem:
    ...
    # It took a while for me to debug, but I mispelled some At() expression as AT()
    # And they were almost invisible when sitting next to airport ATL
    return AirCargoProblem(cargos, planes, airports, init, goal)
