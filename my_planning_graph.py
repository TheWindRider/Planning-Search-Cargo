...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for each_action in self.all_actions: 
            each_node = PgNode_a(each_action)
            its_precond = each_node.prenodes
            # Add action iff all prerequisites hold in previous S level
            if its_precond.issubset(self.s_levels[level]): 
                self.a_levels[level].add(each_node)
                for each_parent in its_precond: 
                    each_parent.children.add(each_node)
                    each_node.parents.add(each_parent)
    ...
    def add_literal_level(self, level):
        ...
        for each_action in self.a_levels[level-1]: 
            its_effect = each_action.effnodes
            # All effect nodes can be added without fear of duplication, thanks to the "Set"
            for each_child in its_effect: 
                self.s_levels[level].add(each_child)
                each_child.parents.add(each_action)
                each_action.children.add(each_child)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        if any(effect in effect_add_a1 for effect in effect_rem_a2): return True
        if any(effect in effect_add_a2 for effect in effect_rem_a1): return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        effect_add_a2 = node_a2.action.effect_add
        effect_rem_a2 = node_a2.action.effect_rem
        if any(effect in precond_neg_a2 for effect in effect_add_a1): return True
        if any(effect in precond_pos_a2 for effect in effect_rem_a1): return True
        if any(effect in precond_neg_a1 for effect in effect_add_a2): return True
        if any(effect in precond_pos_a1 for effect in effect_rem_a2): return True
        return False
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        precond_a1 = node_a1.parents
        precond_a2 = node_a2.parents
        for parent_node_a1 in precond_a1: 
            for parent_node_a2 in precond_a2: 
                if parent_node_a1.is_mutex(parent_node_a2): return True
        # Previous mistake was to test if preconditions "negate" each other
        # But this mutex checking is different, mutex relationship has been built in previos level
        return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for parent_s1 in node_s1.parents: 
            for parent_s2 in node_s2.parents: 
                if not parent_s1.is_mutex(parent_s2): return False
        return True
    ...
    def h_levelsum(self) -> int:
        ...
        goal_set = set([PgNode_s(each_goal, True) for each_goal in self.problem.goal])
        for curr_level in range(len(self.s_levels)): 
            curr_node = self.s_levels[curr_level]
            num_goal_hit = len(goal_set & curr_node) # "=" overloaded in class PgNode_s
            if num_goal_hit == 0: continue
            level_sum += curr_level * num_goal_hit
            goal_set = goal_set - curr_node
        return level_sum
